...
async function main() {
  ...
  const [bookingApproved] = await Booking.create([
    ...
    },
    {
      ...
      status: 'PROPOSED',
      preferredSlots: [{ start: new Date(now + 30 * hour), end: new Date(now + 30.5 * hour) }],
      proposedSlot: { start: new Date(now + 31 * hour), end: new Date(now + 31.5 * hour) },
      clientNote: 'Late afternoon is easier for me.',
      agentNote: 'Can we move this by one hour?',
    },
    {
      propertyId: propertyA._id,
      agentId: agentA._id,
      clientId: clientB._id,
      status: 'DECLINED',
      preferredSlots: [{ start: new Date(now + 36 * hour), end: new Date(now + 36.5 * hour) }],
      clientNote: 'Please confirm as soon as possible.',
      agentNote: 'This slot conflicts with another confirmed appointment.',
    },
    {
      propertyId: propertyB._id,
      agentId: agentA._id,
      clientId: clientA._id,
      status: 'CANCELLED',
      preferredSlots: [{ start: new Date(now + 54 * hour), end: new Date(now + 54.5 * hour) }],
      clientNote: 'Schedule changed. I will rebook later.',
    },
  ]);
  ...
  await Booking.create([
    ...
    },
    {
      ...
      clientId: clientA._id,
      status: 'APPROVED',
      preferredSlots: [{ start: new Date(now + 72 * hour), end: new Date(now + 72.5 * hour) }],
      confirmedSlot: { start: new Date(now + 72 * hour), end: new Date(now + 72.5 * hour) },
      clientNote: 'Any morning slot works.',
      agentNote: 'Confirmed for your requested time.',
    },
    {
      propertyId: propertyC._id,
      agentId: agentB._id,
      clientId: clientB._id,
      status: 'PROPOSED',
      preferredSlots: [{ start: new Date(now + 84 * hour), end: new Date(now + 84.5 * hour) }],
      proposedSlot: { start: new Date(now + 85 * hour), end: new Date(now + 85.5 * hour) },
      clientNote: 'I can do weekdays.',
      agentNote: 'Suggesting this slot based on availability.',
    },
    {
      propertyId: propertyC._id,
      agentId: agentB._id,
      clientId: clientA._id,
      status: 'DECLINED',
      preferredSlots: [{ start: new Date(now + 92 * hour), end: new Date(now + 92.5 * hour) }],
      clientNote: 'Trying to finalize quickly.',
      agentNote: 'Listing unavailable at this time.',
    },
  ]);
